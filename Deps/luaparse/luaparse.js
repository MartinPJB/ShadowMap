/**
 * Original and all the credits to: https://github.com/fstirlitz/luaparse
 * Modified by @MartinPJB to handle basic luau syntax like type definition, ...
 *
 * One nice thing to do is to modify this into OOP and ES2024 style.
 * I can't be arsed rn though.
 */
...
(function (root, name, factory) {
    ...
    var ast = exports.ast = {
        ...
        , localStatement: function (variables, init, typeAnnotations) {
            return {
                type: 'LocalStatement'
                , variables: variables
                , init: init
                , variablesType: typeAnnotations
            };
        }
        ...
        , functionStatement: function (identifier, parameters, isLocal, body, returnType = "any") {
            return {
    ...
    function parseTypeAnnotation() {
        markLocation(); // Track location for the AST node.

        // Start with the first type.
        let type = parseSingleType();

        // Handle union types (`|`).
        while (consume('|')) {
            const nextType = parseSingleType();
            type = finishNode({
    ...
    function parseSingleType() {
        if (Identifier !== token.type) {
            console.log("ID:", Identifier, "TOKEN TYPE:", token.type, "TOKEN:", token);
            raiseUnexpectedToken('<type>', token);
        }

        const baseType = finishNode(ast.typeAnnotation(token.value));
    ...
    function parseFunctionDeclaration(name, isLocal) {
        ...
        return finishNode(ast.functionStatement(name, parameters, isLocal, body, returnType));
    }
